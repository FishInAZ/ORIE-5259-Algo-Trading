...
from leadlag_v1_step3_step4 import TARGET_STOCK, build_signal_panel, compute_equal_weight_signal
...
def prepare_target_panel(lookback_seconds: int) -> pd.DataFrame:
    ...
    panel = panel[panel["stock"] == TARGET_STOCK].copy()
...
def make_test_plots(summary_df: pd.DataFrame, pooled_test: pd.DataFrame) -> tuple[Path, Path]:
    avg_bar_path = STEP89_OUTPUT_DIR / f"test_average_improvement_by_side_{TARGET_STOCK}.png"
    hist_path = STEP89_OUTPUT_DIR / f"test_pooled_improvement_histogram_{TARGET_STOCK}.png"

    fig, ax = plt.subplots(figsize=(6, 4))
    ax.bar(summary_df["side"], summary_df["average_improvement"], color=["#2E86AB", "#C0392B"])
    ax.axhline(0, color="black", linewidth=1)
    ax.set_title(f"Test Average Improvement by Side ({TARGET_STOCK})")
    ax.set_xlabel("Side")
    ...
    ax.set_title(f"Test Pooled Improvement Distribution ({TARGET_STOCK})")
...
def main() -> None:
    ...
    for lookback_seconds, theta_quantile in product(LOOKBACK_GRID, THRESHOLD_QUANTILE_GRID):
        panel = prepare_target_panel(lookback_seconds)
    ...
    if best_result is None or best_panel is None or train_minutes is None or test_minutes is None:
        raise RuntimeError("Parameter search failed to produce a result.")
    ...
    tuning_df.to_csv(STEP89_OUTPUT_DIR / f"train_grid_search_{TARGET_STOCK}.csv", index=False)
    side_summary.to_csv(STEP89_OUTPUT_DIR / f"train_test_side_summary_{TARGET_STOCK}.csv", index=False)
    overall_summary.to_csv(STEP89_OUTPUT_DIR / f"train_test_overall_summary_{TARGET_STOCK}.csv", index=False)
    test_result["buy_cmp"].to_csv(STEP89_OUTPUT_DIR / f"test_buy_strategy_vs_benchmark_{TARGET_STOCK}.csv", index=False)
    test_result["sell_cmp"].to_csv(STEP89_OUTPUT_DIR / f"test_sell_strategy_vs_benchmark_{TARGET_STOCK}.csv", index=False)

    print("STEP 8 SELL SIDE")
    print("=" * 80)
    print("Sell logic implemented:")
    ...
    print()
    print("Saved files:")
    print(STEP89_OUTPUT_DIR / f"train_grid_search_{TARGET_STOCK}.csv")
    print(STEP89_OUTPUT_DIR / f"train_test_side_summary_{TARGET_STOCK}.csv")
    print(STEP89_OUTPUT_DIR / f"train_test_overall_summary_{TARGET_STOCK}.csv")
    print(STEP89_OUTPUT_DIR / f"test_buy_strategy_vs_benchmark_{TARGET_STOCK}.csv")
    print(STEP89_OUTPUT_DIR / f"test_sell_strategy_vs_benchmark_{TARGET_STOCK}.csv")
    print(avg_bar_path)
    print(hist_path)
